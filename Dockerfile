...
RUN wget https://github.com/osTicket/osTicket/releases/download/v1.18.1/osTicket-v1.18.1.zip && \
    ...
    unzip osTicket-v1.18.1.zip -d /var/www/osticket && \
...
RUN sed -i "s/define('DBHOST', 'localhost');/define('DBHOST', getenv('OSTICKET_DB_HOST'));/" /var/www/osticket/upload/include/ost-config.php && \
    sed -i "s/define('DBNAME', 'osticket');/define('DBNAME', getenv('OSTICKET_DB_NAME'));/" /var/www/osticket/upload/include/ost-config.php && \
    sed -i "s/define('DBUSER', 'osticket');/define('DBUSER', getenv('OSTICKET_DB_USER'));/" /var/www/osticket/upload/include/ost-config.php && \
    sed -i "s/define('DBPASS', 'securepassword');/define('DBPASS', getenv('OSTICKET_DB_PASS'));/" /var/www/osticket/upload/include/ost-config.php
